...
int main(int argc, const char* argv[]) {
  NSData* argData = [[NSData alloc]
      initWithBase64EncodedString:[NSString stringWithFormat:@"%s", argv[1]]
                          options:0];
  NSString* json = [[NSString alloc] initWithData:argData
...
void connectToServer(NSDictionary* arguments) {
  ...
  inet_aton([[arguments objectForKey:@"ip"] UTF8String],
  ...
  NSString* jsonString = [[NSString alloc] initWithData:jsonData
                                               encoding:NSUTF8StringEncoding];
  SSL_write(client_ssl, [jsonString UTF8String],
            (int)strlen([jsonString UTF8String]));
  interact(arguments);
}
